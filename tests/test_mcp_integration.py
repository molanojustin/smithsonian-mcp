...
class TestMCPProtocolAdherence:
    ...
    async def test_tool_signature_compliance(self):
        ...
        # Import the server module to check function signatures
        import smithsonian_mcp.server as server_module
        import inspect

        # Get the actual function implementations (the decorated tools are directly callable)
        tools_to_test = [
            ("search_collections", server_module.search_collections),
            ("simple_explore", server_module.simple_explore),
            ("continue_explore", server_module.continue_explore),
            ("get_object_details", server_module.get_object_details),
            ("get_smithsonian_units", server_module.get_smithsonian_units),
            ("get_collection_statistics", server_module.get_collection_statistics),
            ("search_by_unit", server_module.search_by_unit),
            ("get_objects_on_view", server_module.get_objects_on_view),
            ("check_object_on_view", server_module.check_object_on_view),
            ("find_on_view_items", server_module.find_on_view_items),
            ("get_search_context", server_module.get_search_context),
            ("get_object_context", server_module.get_object_context),
            ("get_on_view_context", server_module.get_on_view_context),
            ("get_units_context", server_module.get_units_context),
            ("get_stats_context", server_module.get_stats_context),
        ]
    ...
    async def test_tool_return_types(self):
        ...
        mock_client = AsyncMock(spec=SmithsonianAPIClient)

        with patch('smithsonian_mcp.server.get_api_client', return_value=mock_client):
            ...
            result = await server_module.search_collections.fn(query="test")
            ...
            mock_client.get_units.return_value = []
            result = await server_module.get_smithsonian_units.fn()
            ...
            mock_client.get_collection_stats.return_value = mock_stats
            result = await server_module.get_collection_statistics.fn()
...
class TestEndToEndMCPWorkflows:
    ...
    async def test_full_discovery_workflow(self):
        ...
        with patch('smithsonian_mcp.server.get_api_client', return_value=mock_client):
            ...
            mock_explore_result = SearchResult(
                objects=[
                    SmithsonianObject(
                        id="test-123",
                        title="Test Object",
            ...
            mock_client.search_collections.return_value = mock_explore_result

            explore_result = await server_module.simple_explore.fn(topic="dinosaurs")
            assert explore_result.objects
            object_id = explore_result.objects[0].id

            # Step 2: Get detailed object information
            mock_detail_object = SmithsonianObject(
                id=object_id,
            ...
            mock_client.get_object_by_id.return_value = mock_detail_object

            detail_result = await server_module.get_object_details.fn(object_id=object_id)
            assert detail_result.id == object_id

            # Step 3: Get object context
            context_result = await server_module.get_object_context.fn(object_id=object_id)
    ...
    async def test_exhibition_planning_workflow(self):
        ...
        with patch('smithsonian_mcp.server.get_api_client', return_value=mock_client):
            ...
            mock_client.get_units.return_value = mock_units

            units = await server_module.get_smithsonian_units.fn()
            assert len(units) >= 2

            # Step 2: Find on-view items at specific museum
            mock_on_view_result = SearchResult(
                objects=[
                    SmithsonianObject(
                        id="exhibit-1",
            ...
            mock_client.search_collections.return_value = mock_on_view_result

            on_view_result = await server_module.get_objects_on_view.fn(unit_code="NMAH")
            assert on_view_result.objects
            assert all(obj.is_on_view for obj in on_view_result.objects)

            # Step 3: Get on-view context
            context_result = await server_module.get_on_view_context.fn(unit_code="NMAH")
    ...
    async def test_research_workflow_with_pagination(self):
        ...
        with patch('smithsonian_mcp.server.get_api_client', return_value=mock_client):
            ...
            mock_client.search_collections.return_value = mock_batch1

            first_result = await server_module.simple_explore.fn(topic="fossils", max_samples=5)
            ...
            mock_client.search_collections.return_value = mock_batch2

            continue_result = await server_module.continue_explore.fn(
                topic="fossils",
...
class TestCrossToolDependencies:
    ...
    async def test_search_to_details_dependency(self):
        ...
        with patch('smithsonian_mcp.server.get_api_client', return_value=mock_client):
            ...
            mock_client.search_collections.return_value = mock_search_result

            search_result = await server_module.search_collections.fn(query="technology")
            ...
            mock_detail_object = SmithsonianObject(
                ...
                unit_code="NMAH",
                description="Full description from details API",
            )
            mock_client.get_object_by_id.return_value = mock_detail_object

            detail_result = await server_module.get_object_details.fn(object_id="interconnected-123")
            assert detail_result.id == "interconnected-123"
            assert "Full description from details API" in detail_result.description
    ...
    async def test_unit_search_integration(self):
        ...
        with patch('smithsonian_mcp.server.get_api_client', return_value=mock_client):
            ...
            mock_unit_search_result = SearchResult(
                objects=[
                    SmithsonianObject(
                        id="art-123",
            ...
            mock_client.search_collections.return_value = mock_unit_search_result

            unit_search_result = await server_module.search_by_unit.fn(unit_code="SAAM", query="painting")
    ...
    async def test_on_view_search_reliability(self):
        ...
        with patch('smithsonian_mcp.server.get_api_client', return_value=mock_client):
            ...
            reliable_result = await server_module.find_on_view_items.fn(query="exhibition")
...
class TestErrorHandlingAndEdgeCases:
    ...
    async def test_empty_search_results_handling(self):
        ...
        with patch('smithsonian_mcp.server.get_api_client', return_value=mock_client):
            ...
            mock_client.search_collections.return_value = empty_result

            # Test various tools with empty results
            search_result = await server_module.search_collections.fn(query="nonexistenttopic12345")
            assert search_result.objects == []
            assert search_result.total_count == 0

            explore_result = await server_module.simple_explore.fn(topic="nonexistenttopic12345")
            assert explore_result.objects == []

            on_view_result = await server_module.get_objects_on_view.fn()
            assert on_view_result.objects == []
    ...
    async def test_invalid_object_id_handling(self):
        ...
        with patch('smithsonian_mcp.server.get_api_client', return_value=mock_client):
            ...
            mock_client.get_object_by_id.return_value = None

            result = await server_module.get_object_details.fn(object_id="invalid-id-12345")
            assert result is None

            context_result = await server_module.get_object_context.fn(object_id="invalid-id-12345")
    ...
    async def test_rate_limiting_simulation(self):
        ...
        with patch('smithsonian_mcp.server.get_api_client', return_value=mock_client):
            ...
            mock_client.search_collections.side_effect = Exception("Rate limit exceeded")

            with pytest.raises(Exception) as exc_info:
                await server_module.search_collections.fn(query="test")
...
class TestMCPContextTools:
    ...
    async def test_context_tools_formatting(self):
        ...
        with patch('smithsonian_mcp.server.get_api_client', return_value=mock_client):
            ...
            search_context = await server_module.get_search_context.fn(query="test")
            ...
            assert "ID: ctx-123" in search_context
            ...
            units_context = await server_module.get_units_context.fn()
            assert "Smithsonian Institution Museums" in units_context
            assert "TEST: Test Museum" in units_context
            assert "A test museum" in units_context
    ...
    async def test_on_view_context_filtering(self):
        ...
        with patch('smithsonian_mcp.server.get_api_client', return_value=mock_client):
            ...
            mock_client.search_collections.return_value = mock_result

            on_view_context = await server_module.get_on_view_context.fn()
